...
fun DayBookReportScreen(
    viewModel: DayBookReportViewmodel = hiltViewModel(),
    ...
    LaunchedEffect(fromDate, toDate) {
        ...
        viewModel.loadData(fromDate.format(formatter), toDate.format(formatter))
    }
    Scaffold(
        topBar = {
            TopAppBar(
        ...
        Column(
            ...
            when (val state = ledgerDetailsState) {
                is DayBookReportViewmodel.DayBookUiState.Loading -> {
                    Box(
                ...
                is DayBookReportViewmodel.DayBookUiState.Success -> {
                    LazyColumn(
                        modifier = Modifier
                            .fillMaxSize()
                            .padding(8.dp)
                ...
                is DayBookReportViewmodel.DayBookUiState.Error -> {
                    Box(
                        modifier = Modifier.fillMaxSize(),
                        contentAlignment = Alignment.Center
                    ) {
